...
export default function PrintPhieuXuatPage() {
	const params = useParams();
	const sopx = params.id as string;
	const [phieu, setPhieu] = useState<PhieuXuat | null>(null);
	...
	if (!phieu) {
		return <div className="p-8 text-center text-red-600">Không tìm thấy phiếu xuất</div>;
	}
	...
	return (
		<div className="p-8 max-w-4xl mx-auto bg-white" style={{ printColorAdjust: 'exact' }}>
			...
			<div className="text-center mb-8 border-b pb-4">
				<h1 className="text-2xl font-bold text-gray-800">PHIẾU XUẤT HÀNG</h1>
				<p className="text-sm text-gray-600 mt-2">Số phiếu: {phieu.SoPX}</p>
			</div>
			...
			<div className="mb-6 grid grid-cols-2 gap-4 text-sm">
				<div>
					...
					<span className="ml-2">{phieu.NgayXuat ? new Date(phieu.NgayXuat).toLocaleDateString('vi-VN') : '-'}</span>
				</div>
				<div>
				...
				<tbody>
					{chiTiet.map((ct, i) => (
						<tr key={i}>
							...
							<td className="border border-gray-300 p-2 text-right">{ct.SLXuat}</td>
							<td className="border border-gray-300 p-2 text-right">{Number(ct.DonGia).toLocaleString('vi-VN')}</td>
